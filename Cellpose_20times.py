...
RAW_DIR = BASE_DIR / "Dataset_raw" / "20times"
PROJECT_DIR = BASE_DIR / "Cellpose" / "HeLa_20times"
...
# tubulin guidance
GUIDE_DILATE = 3
GUIDE_MIN_CONTACT_PIXELS = 6
GUIDE_BRIGHT_PERCENTILE = 99.5
...
def normalize_dapi_20x(img):
    ...
    if p_white <= p_black:
        p_white = p_black + 1.0
    arr = (img - p_black) / (p_white - p_black)
    arr = np.clip(arr, 0.0, 1.0)
    arr = arr ** gamma_val
    return (arr * 255).astype(np.uint8)
...
def find_merge_suggestions(mask, tub_img):
    """
    如果两个实例边界邻近，且邻接区域存在格外亮的 tubulin 信号，则给出建议合并。
    """
...
def copy_images_to_run(run_dir: Path):
    ...
    for rid, dna_path, tub_path in pairs:
        ...
        if not out_path.exists():
            img3, _, _ = build_merge_image_20x(dna_path, tub_path)
...
def predict_run(run_dir: Path, model_path):
    ...
    for img_path in sorted(img_dir.glob("*_merge.png"), key=natural_key):
        ...
        dna_path = RAW_DIR / f"{rid}_DNA.jpg"
        if not dna_path.exists():
            ...
            if not dna_candidates:
                ...
                continue
            dna_path = dna_candidates[0]

        img = read_rgb(img_path)
        dapi = normalize_dapi_20x(read_gray(dna_path))

        print(f"[Predict] {img_path.name}")
        masks, flows, styles = model.eval(
            img,
            channels=[2, 3],   # G=tubulin, B=DAPI
            diameter=DIAMETER,
...
def rebuild_train_folder(project_dir: Path, target_run_dir: Path):
    ...
    for run_dir in sorted(project_dir.glob("Run_*"), key=natural_key):
        ...
        for img_path in sorted(img_dir.glob("*_merge.png"), key=natural_key):
            ...
            if not dna_candidates:
                continue
            dapi = normalize_dapi_20x(read_gray(dna_candidates[0]))
...
def train_on_run(run_dir: Path):
    ...
    model_name = f"{run_dir.name}_hela20x"
...
def show_image_blocking(img_name, img_rgb, mask, idx, total, run_name,
    ...
    cv2.imshow("HeLa 20x Cellpose HITL Editor", cv2.cvtColor(canvas, cv2.COLOR_RGB2BGR))
    cv2.waitKey(1)
...
def edit_one_image(run_dir: Path, img_path: Path, idx: int, total: int):
    ...
    cv2.namedWindow("HeLa 20x Cellpose HITL Editor", cv2.WINDOW_NORMAL)
    print_command_help()
...
def main():
    ...
    pairs = find_image_pairs(RAW_DIR)
    print("========================================")
    print("HeLa 20x Cellpose HITL all-in-one")
    print("========================================")
