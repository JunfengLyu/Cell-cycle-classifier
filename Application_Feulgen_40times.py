...
def main():
    ...
    using_finetuned = latest_cellpose_model(CELLPOSE_PROJECT_DIR) is not None
    ...
    for img_path in image_paths:
        ...
        for lid in valid_labels:
            ...
            rows.append({
                ...
                "pred_label": label,
                "pred_num": int(PHASE_TO_NUM[label]),
                "confidence": float(probs_mean[pred_idx]),
                "prob_I": float(probs_mean[0]),
                "prob_P": float(probs_mean[1]),
                "prob_M": float(probs_mean[2]),
                "prob_A": float(probs_mean[3]),
                "prob_T": float(probs_mean[4]),
                "used_guidance_center": False,
                "guidance_source": "centroid",
                "per_model_probs_json": json.dumps(
                    {
                        "fold_votes": [int(v) for v in votes],
        ...
        draw_results(img_rgb, masks, detections, VIS_DIR / f"{rid}_prediction.png")
        ...
        meta = {
            ...
            "classifier": "ResNet34_pretrained_fold_ensemble",
            "box_size": BOX_SIZE,
