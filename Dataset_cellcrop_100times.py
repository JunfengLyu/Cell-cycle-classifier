...
INPUT_DIR = Path("./Dataset_raw/100times")
OUTPUT_DIR = Path("./Dataset/Dataset_100times")
BOX_SIZE = 800
DISPLAY_GAMMA = 1.00
...
def cell_distinguisher_100times():
    input_dir = INPUT_DIR
    ...
    for pair in pairs:
        ...
        dna_main = normalize_image(dna, 1.05, 1, 98.8)
        ...
        window_name = f"100x group: {pair['raw_id']}"
        mouse = MouseState()
        ...
        while True:
            ...
            if mouse.clicked:
                ...
                keep_cell, phase = review_cell_100times(tub_crop, dna_crop)
                if not keep_cell:
                    continue
                ...
                Image.fromarray(normalize_image(dna_crop, 1.05, 1, 98.8)).save(dna_out)
...
def review_cell_100times(tub_crop, dna_crop):
    tub8 = normalize_image(tub_crop, 1.0, 1, 99.5)
    dna8 = normalize_image(dna_crop, 1.05, 1, 98.8)
    ...
    preview_name = "100x Candidate preview"
